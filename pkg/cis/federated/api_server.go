...
func APIServer(index, subIndex int, missingProcessFunc framework.MissingProcessHandlerFunc) {
	...
	It(fmt.Sprintf("[%d.%d.4] Ensure that the --insecure-bind-address argument is not set", index, subIndex), func() {
		ExpectProcess(f).To(NotHaveFlagOrHaveFlagWithValue("--insecure-bind-address", "127.0.0.1"))
	})

	It(fmt.Sprintf("[%d.%d.5] Ensure that the --insecure-port argument is set to 0", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagWithValue("--insecure-port", "0"))
	})

	It(fmt.Sprintf("[%d.%d.6] Ensure that the --secure-port argument is not set to 0", index, subIndex), func() {
		ExpectProcess(f).To(NotHaveFlagOrHaveFlagWithDifferentValue("--secure-port", "0"))
	})

	It(fmt.Sprintf("[%d.%d.7] Ensure that the --profiling argument is set to false", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagWithValue("--profiling", "false"))
	})

	It(fmt.Sprintf("[%d.%d.8] Ensure that the admission control policy is not set to AlwaysAdmit", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagThatDoesNotContainValue("--admission-control", "AlwaysAdmit"))
	})

	It(fmt.Sprintf("[%d.%d.9] Ensure that the admission control policy is set to NamespaceLifecycle", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagThatContainsValue("--admission-control", "NamespaceLifecycle"))
	})

	It(fmt.Sprintf("[%d.%d.10] Ensure that the --audit-log-path argument is set as appropriate", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagWithAnyValue("--audit-log-path"))
	})

	It(fmt.Sprintf("[%d.%d.11] Ensure that the --audit-log-maxage argument is set to 30 or as appropriate", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagThatMatchesNumerically("--audit-log-maxage", ">=", 30))
	})

	It(fmt.Sprintf("[%d.%d.12] Ensure that the --audit-log-maxbackup argument is set to 10 or as appropriate", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagThatMatchesNumerically("--audit-log-maxbackup", ">=", 10))
	})

	It(fmt.Sprintf("[%d.%d.13] Ensure that the --audit-log-maxsize argument is set to 100 or as appropriate", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagThatMatchesNumerically("--audit-log-maxsize", ">=", 100))
	})

	It(fmt.Sprintf("[%d.%d.14] Ensure that the --authorization-mode argument is not set to AlwaysAllow", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagWithDifferentValue("--authorization-mode", "AlwaysAllow"))
	})

	It(fmt.Sprintf("[%d.%d.15] Ensure that the --token-auth-file parameter is not set", index, subIndex), func() {
		ExpectProcess(f).To(NotHaveFlag("--token-auth-file"))
	})

	It(fmt.Sprintf("[%d.%d.16] Ensure that the --service-account-lookup argument is set to true", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagWithValue("--service-account-lookup", "true"))
	})

	It(fmt.Sprintf("[%d.%d.17] Ensure that the --service-account-key-file argument is set as appropriate", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagWithAnyValue("--service-account-key-file"))
	})

	It(fmt.Sprintf("[%d.%d.18] Ensure that the --etcd-certfile and --etcd-keyfile arguments are set as appropriate", index, subIndex), func() {
		ExpectProcess(f).To(HaveFlagWithAnyValue("--etcd-certfile"))
		ExpectProcess(f).To(HaveFlagWithAnyValue("--etcd-keyfile"))
	})

	It(fmt.Sprintf("[%d.%d.19] Ensure that the --tls-cert-file and --tls-private-key-file arguments are set as appropriate", index, subIndex), func() {
